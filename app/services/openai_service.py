    ...
    async def classify_intent(self, message: str, context: dict = None,user_phone=None) -> Dict[str, Any]:
        ...
        try:
            ...
            if usage:
                ...
                token_msg = f"OpenAI API call time: {api_call_time:.2f}s | Input tokens: {total_input_tokens} | Cached: {cached_tokens} ({cache_percentage:.1f}%) | Output: {output_tokens}"
                logger.info(f"[TOKEN_USAGE] [INTENT_CLASSIFICATION] {token_msg}")
            else:
    ...
    async def extract_entities(self, message: str, workflow_type: str = "product_search") -> Dict[str, Any]:
        ...
        try:
            ...
            if usage:
                ...
                cache_percentage = (cached_tokens / total_input_tokens * 100) if total_input_tokens > 0 else 0
                token_msg = f"OpenAI API call time: {api_call_time:.2f}s | Input tokens: {total_input_tokens} | Cached: {cached_tokens} ({cache_percentage:.1f}%) | Output: {output_tokens}"
                logger.info(f"[TOKEN_USAGE] [ENTITY_EXTRACTION] {token_msg}")
                print(f"[ENTITY_EXTRACTION] {token_msg}")
            else:
                logger.info(f"OpenAI API call time: {api_call_time:.2f}s (no usage data available)")
    ...
    async def extract_entities_with_summary_context(self, message: str, chat_summaries: List[Dict], workflow_type: str = "rfq_creation", existing_products: List[Dict] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if usage:
                ...
                cache_percentage = (cached_tokens / total_input_tokens * 100) if total_input_tokens > 0 else 0
                token_msg = f"OpenAI API call time: {api_call_time:.2f}s | Input tokens: {total_input_tokens} | Cached: {cached_tokens} ({cache_percentage:.1f}%) | Output: {output_tokens}"
                logger.info(f"[TOKEN_USAGE] [SUMMARY_AWARE_EXTRACTION] {token_msg}")
                print(f"[SUMMARY_AWARE_EXTRACTION] {token_msg}")
            else:
    ...
    async def parse_seller_product_items(self, details: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if usage:
                ...
                cache_percentage = (cached_tokens / total_input_tokens * 100) if total_input_tokens > 0 else 0
                token_msg = f"Parse seller products API call: {api_call_time:.2f}s | Input: {total_input_tokens} | Cached: {cached_tokens} ({cache_percentage:.1f}%) | Output: {output_tokens}"
                logger.info(f"[TOKEN_USAGE] [PARSE_SELLER_PRODUCTS] {token_msg}")
                print(f"[PARSE_SELLER_PRODUCTS] {token_msg}")
...
"""
            ...
            if response.output and len(response.output) > 0:
                ...
                if function_call.type == "function_call":
                    ...
                    if usage:
                        ...
                        print(f"[OPENAI_SERVICE] 3-level categorization token usage: {token_usage}")
                ...
                if function_call.type == "function_call":
                    ...
                    if selected_category:
                        ...
                        if usage:
                            ...
                            print(f"[OPENAI_SERVICE] Token usage for seller category mapping: {token_usage}")
    ...
    async def map_seller_categories_batch(
        self,
        seller_categories: List[str],
        existing_categories: List[Dict[str, Any]]
    ) -> Dict[str, Dict[str, Any]]:
        """
        Map multiple seller categories to learning categories in a single OpenAI call.

        Returns:
            Dict mapping seller_category_name -> mapping result
        """
        start_time = time.time()

        try:
            # Load batch mapping tool
            with open(self.tools_dir / "seller_batch_category_mapping.json", 'r') as f:
                mapping_tool = json.load(f)

            # Deduplicate to unique level_1 > level_2 pairs
            l1_l2_map = {}
            for cat in existing_categories:
                key = (cat['level_1_category'], cat['level_2_category'])
                if key not in l1_l2_map:
            ...
            deduplicated_categories = list(l1_l2_map.values())
            logger.info(f"[BATCH] Mapping {len(seller_categories)} categories "
                        f"(deduplicated {len(existing_categories)} -> {len(deduplicated_categories)} unique L1>L2 pairs)")

            # Build context
            context_text = "Seller categories to map:\n"
            for i, cat in enumerate(seller_categories, 1):
                context_text += f"{i}. \"{cat}\"\n"
            ...
            for attempt in range(max_retries):
                try:
                    response = await self.client.responses.create(
                        model=self.default_model,
                        input=[{"role": "user", "content": context_text}],
                        instructions=self._load_prompt("seller_mapping", "_map_seller_to_existing_categories_batch"),
                        tools=[mapping_tool],
                        tool_choice={"type": "function", "name": "select_existing_categories_batch"}
                    )
                    break
                except RateLimitError as e:
                    wait_time = 2 ** attempt
                    logger.warning(f"[BATCH] Rate limit hit (attempt {attempt + 1}/{max_retries}), waiting {wait_time}s...")
                    await asyncio.sleep(wait_time)
                    if attempt == max_retries - 1:
                        raise
            ...
            output_tokens = getattr(usage, 'output_tokens', 0) if usage else 0
            total_tokens = input_tokens + output_tokens
            logger.info(f"[BATCH] Token usage: input={input_tokens}, output={output_tokens}, total={total_tokens}")
            ...
            if response.output and len(response.output) > 0:
                ...
                if function_call.type == "function_call":
                    ...
                    for mapping in mappings:
                        cat_name = mapping.get("seller_category")
                        selected_id = mapping.get("selected_category_id")

                        # Find the category in original list
                        selected_category = None
                        for cat in existing_categories:
                        ...
                        if selected_category:
                            results[cat_name] = {
                                "success": True,
                                "selected_category": selected_category,
                                "similarity_score": mapping.get("similarity_score", 0.8),
                                "reasoning": mapping.get("reasoning", ""),
                                "processing_time_ms": int(processing_time * 1000),
                                "token_usage": {"input_tokens": input_tokens, "output_tokens": output_tokens, "total_tokens": total_tokens}
                            }
                        else:
                            logger.error(f"[BATCH] Category ID '{selected_id}' not found for '{cat_name}'")
                            results[cat_name] = {
                                "success": False,
                                "error": f"Selected category ID {selected_id} not found",
                                "token_usage": {"input_tokens": input_tokens, "output_tokens": output_tokens, "total_tokens": total_tokens}
                            }

            logger.info(f"[BATCH] Mapped {len([r for r in results.values() if r.get('success')])} / {len(seller_categories)} categories successfully")
            return results

        except Exception as e:
            logger.error(f"[BATCH] Seller category batch mapping failed: {str(e)}")
            return {cat: {"success": False, "error": str(e)} for cat in seller_categories}
