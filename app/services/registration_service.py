    ...
    async def handle_registration_otp_validation(self, user_phone: str, message_content: str,
        ...
        try:
            ...
            if otp_result.get("status") == "otp_valid":
                ...
                # CRITICAL: After successful OTP, refresh user data and check verification status
                logger.info(f"OTP validated successfully for registration, checking verification status for {user_phone}")
                
                # Get fresh user data from API to check verification status
                from app.procucev_apis.auth_apis import AuthAPIService
                auth_api_service = AuthAPIService()
                auth_response = await auth_api_service.authenticate_user(user_phone)
                
                if auth_response.get("success") and auth_response.get("data"):
                    fresh_users = auth_response["data"]
                    selected_user = fresh_users[0] if isinstance(fresh_users, list) else fresh_users
                    
                    # Check verification status including approved flag
                    from app.services.verification_check_service import VerificationCheckService
                    verification_service = VerificationCheckService(auth_api_service, self.otp_service, self.whatsapp_service)
                    verification_check = await verification_service.check_and_enforce_verification(user_phone, selected_user)
                    
                    if not verification_check.get("access_granted"):
                        # User doesn't meet verification requirements (e.g., approved=False for buyers)
                        logger.info(f"User {user_phone} blocked after registration OTP due to verification requirements: {verification_check}")
                ...
                if user_type == "buyer":
                    # Buyers: Registration completed, verification will be checked during authentication
                    success_message = (
                        "Registration successful—thank you! Please log in again to access your account."
                    )
                    ...
                    # Clear the workflow completely to prevent further processing
                    ...
                    # Sellers: Registration completed, verification will be checked during authentication
                    success_message = (
                        "Registration successful! Please log in again to access your account."
                    )
