...
class WhatsAppService:
    ...
    async def send_configurable_buttons(self,
        ...
        try:
            ...
            logger.debug(f"Sending buttons to {recipient_id}: {button_titles}")
            ...
            logger.debug(f"Button payload: {content}")
    ...
    def _handle_api_response(self, response: requests.Response) -> MessageResponse:
        ...
        try:
            ...
            logger.debug(f"[WHATSAPP_API] Response status: {response.status_code}")
            try:
                logger.debug(f"[WHATSAPP_API] Response body: {response.text[:500]}")
            except Exception:
                pass
    ...
    def _format_phone_number(self, phone: str) -> str:
        ...
        logger.debug(f"Formatted phone number: {phone[:5]}...{phone[-3:]} (length: {len(phone)})")
        return phone