...
class LogCleanupManager:
    ...
    # Patterns for date-based log files in main logs directory
    LOG_PATTERNS = [
    ...
    # Subdirectories with their specific patterns
    LOG_SUBDIRS = [
        ('app', [
            r'^app_\d{4}-\d{2}-\d{2}\.log$',      # legacy
            r'^app\.log\.\d{4}-\d{2}-\d{2}$',     # current
        ]),
        ('celery_worker', [
            r'^celery_worker_\d{4}-\d{2}-\d{2}\.log$',
            r'^critical_pool_\d{4}-\d{2}-\d{2}\.log$',  # New: Critical pool logs
            r'^operations_pool_\d{4}-\d{2}-\d{2}\.log$',  # New: Operations pool logs
            r'^bulk_pool_\d{4}-\d{2}-\d{2}\.log$',  # New: Bulk pool logs
        ]),
        ('celery_beat', [r'^celery_beat_\d{4}-\d{2}-\d{2}\.log$']),
        ('chromadb', [r'^chromadb_\d{4}-\d{2}-\d{2}\.log$']),
        ('openai_interactions', [r'^interactions_\d{4}-\d{2}-\d{2}\.jsonl$', r'^openai_interaction_\d{4}-\d{2}-\d{2}\.log$']),
        ('procucev_api_interactions', [r'^procucev_api_calls_\d{4}-\d{2}-\d{2}\.jsonl$', r'^procucev_api_\d{4}-\d{2}-\d{2}\.log$']),
        ('whatsapp', [r'^webhook_\d{4}-\d{2}-\d{2}\.log$', r'^media_\d{4}-\d{2}-\d{2}\.log$']),
        ('notifications', [r'^notifications_\d{4}-\d{2}-\d{2}\.log$']),
        ('api_health_monitor', [r'^api_health_monitor_\d{4}-\d{2}-\d{2}\.log$']),
    ]
    ...
    def _find_old_logs(self) -> Dict[str, List[Path]]:
        ...
        for subdir_name, patterns in self.LOG_SUBDIRS:
            ...
            for pattern in patterns:
                ...
                # '*.log*' matches both old-style (foo_YYYY-MM-DD.log) and
                # new-style (foo.log.YYYY-MM-DD) names.
                file_ext = '*.jsonl' if 'jsonl' in pattern else '*.log*'

                for log_file in subdir.glob(file_ext):
                    if re.match(pattern, log_file.name):
                        if self._should_exclude(log_file.name):
                            logger.debug(f"Excluded (pattern match): {log_file.name}")
                            continue
...
@shared_task(
    ...
    time_limit=900,       # 15 min max (runaway protection)
...
def cleanup_logs(self):
    ...
    try:
        ...
        # Rely on autoretry_for - will retry up to 3 times
        raise
