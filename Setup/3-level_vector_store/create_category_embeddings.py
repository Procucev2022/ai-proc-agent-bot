...
def create_unified_vector_store(clear_existing: bool = False):
    ...
    # Connect to ChromaDB using PersistentClient with Settings (v2 API)
    # This automatically unloads unused collections when memory limit is reached
    chroma_client = get_chroma_client()
    embedding_function = embedding_functions.SentenceTransformerEmbeddingFunction(model_name="all-MiniLM-L6-v2")
    logger.info(f"Connected to ChromaDB PersistentClient at {settings.chroma_persist_directory}")
    ...
    try:
        ...
        for batch_start in range(0, len(documents), chroma_batch_size):
            batch_end = min(batch_start + chroma_batch_size, len(documents))
            logger.info(f"  Upserting batch {batch_start // chroma_batch_size + 1}/{(len(documents) + chroma_batch_size - 1) // chroma_batch_size}: items {batch_start}-{batch_end}")
            collection.add(
